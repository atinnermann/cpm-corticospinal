...
for sub = 1:nSubs
    ...
    names       = {'PhasicP','PhasicC','Rating','Scale','TonicP','TonicC'};
    onsets      = [{onsPp+shift},{onsPw+shift},{onsVAS},{onsVAS+vasRT},{onsTp},{onsTw}];
    durations   = [{phasicDur},{phasicDur},{vasRT},{scaleDur-vasRT},{tonicDur},{tonicDur}]; %durPhasicR

    %save one file per session
    outName = sprintf('sub%02d_multicond_onsP_cc', subIDs(sub));
