...
for sub = 1:nSubs
    ...
    names       = {'Phasic','Rating','Scale','TonicP','TonicC'};
    onsets      = [{onsP+shift},{onsVAS},{onsVAS+vasRT},{onsTp},{onsTw}];
    durations   = [{phasicDur},{vasRT},{scaleDur-vasRT},{tonicDur},{tonicDur}];
    

    %save one file per session
    outName = sprintf('sub%02d_multicond_onsP_cc_ph1reg_new', subIDs(sub));
