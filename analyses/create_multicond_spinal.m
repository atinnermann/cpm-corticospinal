function create_multicond_spinal
...
for sub = 1:nSubs
    ...
    for run = 1:nRuns
        ...
        onsets      = [{onsPhasicP+shift},{onsVAS},{onsTonic}];
        durations   = [{phasicDur},{vasRT},{tonicDur}]; %durPhasicR 

        %save one file per session
        outName = sprintf('sub%02d_multicond_onsP_sp_run%d', subIDs(sub),run);
