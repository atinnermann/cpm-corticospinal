function [problems] = first_level_brain_ppi
...
exclude = [3 7 14 19 28 35 36 41 25 24];
...
fLevelVoi      = 'first_level_spinal_hrf_cc';
if isfield(xY,'thresh')
    ppiName        = sprintf('ppi_brain_tc_%s_%dmm_0%d_c%d',xY.str,xY.rad,100*xY.thresh,xY.T);
else
    ppiName        = sprintf('ppi_brain_tc_%s_%dmm',xY.str,xY.rad);
end
model          = 1;
smooth         = 1;
...
if model == 1
    for sub = 1:nSubs
        ...
        mask        = fullfile(anatDir,spm_file(sprintf(vars.T1stripID,subIDs(sub)),'prefix','w'));
        ...
        VOI = get_timeseries(fullfile(voiDir,'SPM.mat'),meanEpiSp,[],vars.smKernSp,xY); %
        ...
        for cr = 1:numel(bvreg)
            if ~isempty(bvreg{cr})
                bv = zeros(nScans,size(bvreg{cr},2));
