...
nSess          = vars.nSess;
nRuns          = vars.nRuns;
nDummies       = vars.nDummy;
TR             = vars.repTime;
incomplRuns    = 200; %rule for number of scans provided in get_study_specs for data import to find incomplete runs

for sub = 1:nSubs
    for ses = 1:nSess
        subDir = fullfile(physioDir,sprintf('sub%02d',subIDs(sub)),'physio');
        fileName = fullfile(subDir,sprintf('sub%02d_physio.mat',subIDs(sub)));
        ...
        if do_preproc
            ...
            for run = 1:nRuns
                ...
                saveName = fullfile(subDir,sprintf('sub%02d_physio_run%d.tsv',subIDs(sub),run));
