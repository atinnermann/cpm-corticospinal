function y = get_roi_ts(xY, V_epi, V_wmean_scan,V_part) % N_back_scan,
...
XYZvox = inv(V_part.mat)*[XYZmm; ones(1,size(XYZmm,2))];
XYZvox       = XYZvox(1:3,:); %map mm to vox
   

% [oXYZvox, ~]  = transform_back(XYZmm, V_wmean_scan, V_epi(1), N_back_scan);

y         = spm_get_data(V_epi,XYZvox); % get relevant EPI time-series