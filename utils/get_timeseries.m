function temp_xY = get_timeseries(SPM, wmean_file, back_file, sk, xY)
...
for i = 1:numel(xY) %loop across all ROIs
    ...
    if isfield(xY(i),'T')
        ...
        V_T_df = str2num(cell2mat(extractBetween(V_T.descrip,"[","]")));
        if sk > 0
            V_T = spm_smoothto16bit_s(V_T,sk);
        end
        if ~isfield(xY(i),'thresh')
            xY(i).xyz_o  = xY(i).xyz;
            xY(i).xyz    = get_max_coord(xY(i),V_T, V_wmean_scan, N_back_scan, xY(i).rad2);
        end
    else
        V_T = [];
        V_T_df = [];
    ...
    [y, xyz]   = get_ts(SPM,xY(i),V_epi, V_wmean_scan, N_back_scan, V_beta,V_T,V_T_df);
    y_fin      = (~any(~isfinite(y))) & (~(sum(y)==0));  %if outside mask remove NaNs and columns of zeros!!
%     if sum(~y_fin)>0
%         keyboard; %let's see
%     end
    y        = y(:,y_fin);
    fprintf('Number of voxels found in ROI: %d\n',size(y,2));
...
%now save
% mat_f = [SPM.swd filesep xY(1).str '.mat'];
% mat_f = [];
% save(mat_f,'data');
...
function [y, XYZ] = get_ts(SPM,xY,V_epi, V_wmean_scan, N_back_scan, V_beta,V_T,V_T_df)
...
if isfield(xY,'thresh')
    if size(V_T_df,2) == 2
        newThresh = spm_invFcdf(1-xY.thresh,V_T_df(1),V_T_df(2));
    else
        newThresh = spm_invTcdf(1-xY.thresh,V_T_df);
    end   
   t = spm_get_data(V_T,oXYZvox);
   oXYZvox = oXYZvox(:,t>newThresh);
end
