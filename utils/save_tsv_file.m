...
spm_jsonwrite(spm_file(filename,'ext','.json'),bold_json, struct('indent','  '));

matrix = [crop.cardiac,crop.resp,crop.trigger];
writematrix(matrix,spm_file(filename,'ext','.tsv'),'FileType','text','Delimiter','\t');
gzip(spm_file(filename,'ext','.tsv'));
