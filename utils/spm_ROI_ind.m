function [xY, XYZmm, j] = spm_ROI_ind(xY, XYZmm)
...
%
%    xY.ind      - if xY.def == 'mask' specifies the value of the mask
%                  useful for atlas images 
%
...
%
% modified by CB to allow for indexed mask
...
switch lower(xY.def)
    ...
    if spm_mesh_detect(xY.spec)
        ...
        if isfield(xY,'ind') % added by CB to specify which intensity is used
            j   = find(spm_sample_vol(xY.spec, XYZ(1,:), XYZ(2,:), XYZ(3,:),0) == xY.ind);
        else
